...
create table if not exists review_game_logs (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references students(id) on delete cascade,
  level text not null,
...
create index if not exists students_teacher_login_idx on students (teacher_id, login_id, password);
create unique index if not exists students_teacher_legacy_idx on students (teacher_id, legacy_id) where legacy_id <> '';
create index if not exists curriculum_level_day_idx on curriculum_days (level, day);
create index if not exists progress_student_day_idx on student_progress (student_id, curriculum_day_id);
create index if not exists student_wrong_words_student_day_idx on student_wrong_words (student_id, curriculum_day_id);
create index if not exists vocab_word_idx on vocab_items (word);
